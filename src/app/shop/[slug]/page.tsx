...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-8 py-8">
      <div className="flex gap-2 items-center text-sm text-muted mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-white rounded-3xl border border-border p-8 mb-12">
        <div className="bg-yellow-3 rounded-2xl aspect-square flex items-center justify-center text-[12rem] relative overflow-hidden">
          {product.imageUrl ? (
        ...
        <div>
          ...
          <h1 className="font-serif text-4xl leading-tight mb-2 text-dark">{product.name}</h1>
          <p className="text-muted leading-relaxed mb-6">{product.tasteNote}</p>

          <div className="text-3xl font-bold text-green mb-8">
            ₹{activeVariant.price / 100} 
            <span className="text-sm font-normal text-muted ml-2">per {activeVariant.weight} kg box</span>
          </div>
          ...
          <div className="bg-cream rounded-xl p-5 mb-8 border border-border/50">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <button 
              ...
              className="flex-1 bg-green text-white rounded-full py-3.5 px-8 font-semibold flex items-center justify-center gap-2 hover:bg-green-2 hover:-translate-y-0.5 transition-all shadow-[0_4px_12px_rgba(26,92,42,0.2)]"
            >
              <ShoppingCart size={18} /> Add to Cart — ₹{(activeVariant.price * quantity) / 100}
            </button>
          </div>

          <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm pt-6 border-t border-border">
            ...
            <div className="flex justify-between border-b border-border/50 pb-2 col-span-2">
      ...
      {product.nutritionInfo && (
        <div className="bg-white rounded-3xl border border-border p-8 mb-12">
